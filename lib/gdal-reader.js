...
const GDALINFO_EXE = isWindows 
  ...
  : 'gdalinfo'  // Use system PATH for Linux
const GDAL_TRANSLATE_EXE = isWindows 
  ? path.join(GDAL_PATH, 'gdal_translate.exe')
  : 'gdal_translate'  // Use system PATH for Linux

// Global variables to store found GDAL paths
let foundGdalInfoPath = null
let foundGdalTranslatePath = null

// Check if GDAL is available and find the correct paths
async function checkGDALAvailability() {
  try {
    if (isWindows) {
      // For Windows, use the specific path
      try {
        await executeGDAL(GDALINFO_EXE, ['--version'])
        console.log(`Found GDAL at: ${GDALINFO_EXE}`)
        foundGdalInfoPath = GDALINFO_EXE
        foundGdalTranslatePath = GDAL_TRANSLATE_EXE
        return true
      } catch (error) {
        console.warn('GDAL not available on Windows:', error.message)
        return false
      }
    } else {
      // For Linux (Railway), try common locations
      const commonPaths = [
        'gdalinfo',
        '/usr/bin/gdalinfo',
      ...
      for (const gdalPath of commonPaths) {
        try {
          await executeGDAL(gdalPath, ['--version'])
          console.log(`Found GDAL at: ${gdalPath}`)
          
          // Set the found paths
          foundGdalInfoPath = gdalPath
          foundGdalTranslatePath = gdalPath.replace('gdalinfo', 'gdal_translate')
      ...
      console.warn('GDAL not found in any common location on Linux')
      return false
    }
  } catch (error) {
...
export async function extractPixelDataWithCoordinates(filePath) {
  try {
    ...
    const gdalInfoCmd = foundGdalInfoPath || GDALINFO_EXE
    const infoOutput = await executeGDAL(gdalInfoCmd, [absolutePath])
    ...
    try {
      ...
      const gdalTranslateCmd = foundGdalTranslatePath || GDAL_TRANSLATE_EXE
      await executeGDAL(gdalTranslateCmd, [
